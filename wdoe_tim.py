...
if args.dataset == 'cifar10':
    ...
    cifar_data = dset.CIFAR100('../data/cifarpy', train=False, transform=test_transform, download=True) 
    num_classes = 10
# else:
...
def train(epoch, diff):
    ...
    for batch_idx, (in_set, out_set) in enumerate(zip(train_loader_in, train_loader_out)):
        ...
        if epoch >= args.warmup:
            ...
            proxy.train()
            org_embs = net.feature_list(data)[1].detach()[len(in_set[0]):]
            ...
            (reg_sur + args.gamma * l_reg).backward() # left
            torch.nn.utils.clip_grad_norm_(net.parameters(), 1)
            proxy_optim.step()
            if epoch == args.warmup and batch_idx == 0:
                diff = awp.diff_in_weights(net, proxy)
            else:
                diff = awp.average_diff(diff, awp.diff_in_weights(net, proxy), beta = 0.6)
...
scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda step: cosine_annealing(step, 20 * len(train_loader_in), 1, 1e-6 / args.learning_rate))
diff = None
